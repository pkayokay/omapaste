...
**Status:** inactive. The product path is the Quattro plugin (QML + `capture.sh` / `paste.sh` / `history.py`).
...
| `~/.local/share/omapaste/history.sqlite` | `~/.local/state/omapaste/history.sqlite` |
